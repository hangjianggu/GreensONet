...
import time
import os
from problem import Stokes, Poisson
...
class Tester(object):
    ...
    def calculate_block(self, k):
        ...
        for case_index in range(15,16):
            ...
            #             self.net_pde.G[i][j].load_state_dict(self.checkpoint['state_dict'][i + j])
            ...
            print(f'Finish block {k}, spent time: {time.time()-tt}, loss: {loss.item()}')
    ...
    def save_to_tecplot(self, case_index):
        sorted_data = []
        ...
        for node in self.mesh.vertices:
            if any(np.allclose(node, coord) for coord in self.result['coord']):
                ...
                sorted_data.append(
                    [node[0], node[1], node[2], self.result['u_pred'][index][0], self.result['u_exac'][index][0], self.result['error'][index][0]])
            else:
                boundary_point_value = self.problem.u_exact(node[np.newaxis, ...], case_index)
                sorted_data.append([node[0], node[1], node[2], boundary_point_value[0][0], boundary_point_value[0][0], 0])

        sorted_data = np.array(sorted_data)
        file_path = f'results/{self.args.pde_case}_solution_case{self.args.solution_case}.txt'
        with open(file_path, 'w') as file:
            file.write("Variables=x,y,z,u_exac,u_pred,error\n")
            file.write(f"Zone n={len(self.mesh.vertices)}, e={len(self.mesh.elements)}, f=fepoint, et=tetrahedron\n")
            np.savetxt(file, sorted_data, fmt='%.18e')
            np.savetxt(file, self.mesh.elements, fmt='%d')
    ...
    def save_to_vtk(self, case_index):
        ...
        mesh = meshio.Mesh(
            ...
            point_data={"v_pred": sorted_data[:, 3], "v_exac": sorted_data[:, 4], "v_error": sorted_data[:, 5]}
        )

        if not os.path.exists(os.path.join('results', f'{self.args.pde_case}')):
            os.mkdir(os.path.join('results', f'{self.args.pde_case}'))
        if not os.path.exists(os.path.join(f'results/{self.args.pde_case}', f'{self.args.geo_type}')):
            os.mkdir(os.path.join(f'results/{self.args.pde_case}', f'{self.args.geo_type}'))

        mesh.write( f'results/{self.args.pde_case}/{self.args.geo_type}/case{case_index+1}.vtu')
    ...
    def visual_solution(self, case_index):
        ...
        mesh.write( f'results/{self.args.pde_case}_{self.args.geo_type}_visual_solution_case{self.args.solution_case}.vtk', binary=False)
...
if __name__ == '__main__':
    ...
    if args.pde_case == 'Stokes':
        args.problem = Stokes('data/stokes/test_data')
    elif args.pde_case == 'Poisson':
        args.problem = Poisson('data/poisson/test_data')

    # args.layers = [6, 9, 20, 20, 1]
    # args.layers = [6, 24, 24, 24, 24, 1]
    args.layers = [6, 24, 24, 24, 1]

    args.model_path = f'checkpoints/{args.pde_case}/{args.geo_type}/block0.pkl'
