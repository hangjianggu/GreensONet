...
def U_exact(x, case_index):
    ...
    interpolated_u = griddata(points=points, values=u_exact, xi=x, method='nearest')
    v_exact = mesh.point_data.get("v", None)
    interpolated_v = griddata(points=points, values=v_exact, xi=x, method='nearest')
    w_exact = mesh.point_data.get("w", None)
    interpolated_w = griddata(points=points, values=w_exact, xi=x, method='nearest')

    # U = (u_exact**2 + v_exact**2 + w_exact**2)**0.5
    # interpolated_values = griddata(points=points, values=U, xi=x, method='nearest')

    res = np.column_stack((interpolated_u, interpolated_v, interpolated_w))
    return res
...
def save_to_vtk(vertices, xyz, elements, u_pred_, u_exact_):
    u_pred = (u_pred_[...,0]**2+u_pred_[...,1]**2+u_pred_[...,2]**2)**0.5
    u_exact = (u_exact_[...,0]**2+u_exact_[...,1]**2+u_exact_[...,2]**2)**0.5

    interpolated_u_pred = griddata(points=xyz, values=u_pred.flatten(), xi=vertices, method='nearest')
    interpolated_u_exact = griddata(points=xyz, values=u_exact.flatten(), xi=vertices, method='nearest')
    ...
    mesh.write('case16_fno.vtu')


def save_to_vtk2(vertices, xyz, elements, u_pred):
    interpolated_u_pred = griddata(points=xyz, values=u_pred[:,:,:,0].flatten(), xi=vertices, method='nearest')
    interpolated_v_pred = griddata(points=xyz, values=u_pred[:,:,:,1].flatten(), xi=vertices, method='nearest')
    interpolated_w_pred = griddata(points=xyz, values=u_pred[:,:,:,2].flatten(), xi=vertices, method='nearest')

    mesh = meshio.Mesh(
        points = vertices,
        cells = [("tetra", elements)],
        point_data={"fno_u": interpolated_u_pred, "fno_v": interpolated_v_pred, "fno_w": interpolated_w_pred}
    )
    mesh.write('case16_fno_streamline.vtu')
...
for case_index in range(15, 20):
    ...
    BC_data.append(boundary_v_mag(xyz).reshape((npoints, npoints, npoints, -1)))
...
with torch.no_grad():
    t1 = time.time()
    output = fno(inputs)
    output_ = (output[...,0]**2+output[...,1]**2+output[...,2]**2)**0.5
    targets_ = (targets[...,0]**2+targets[...,1]**2+targets[...,2]**2)**0.5
    loss = criterion(output_, targets_)
    t2 = time.time()
    print(loss.item(), t2-t1)
    save_to_vtk(vertices, xyz, elements, output[0, ...].detach().cpu().numpy(), targets[0, ...].detach().cpu().numpy())
    save_to_vtk2(vertices, xyz, elements, output[0, ...].detach().cpu().numpy())