...
def save_to_vtk(vertices, elements, u_pred, u_exact):
    ...
    mesh.write('case16_deeponet.vtu')


def save_to_vtk2(vertices, elements, u_pred):
    mesh = meshio.Mesh(
        points = vertices,
        cells = [("tetra", elements)],
        point_data={"deeponet_u": u_pred[:,0], "deeponet_v": u_pred[:,1], "deeponet_w": u_pred[:,2]}
    )
    mesh.write('case16_deeponet_streamline.vtu')
...
model_path = 'deeponet_model_uvw.pth'
...
with torch.no_grad():
    ...
    save_to_vtk2(vertices, elements, output[0, ...].detach().cpu().numpy())