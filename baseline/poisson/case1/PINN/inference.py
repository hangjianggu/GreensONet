...
def main():
    ...
    pde_loss = poisson_loss(model, x, f, boundary_x, boundary_u)
    ...
    u_exact_ = torch.cat((torch.tensor(u_exact, dtype=torch.float32), boundary_u[..., None]), dim=0)
    u_pred_ = torch.cat((u_pred, boundary_u_pred), dim=0)
    loss = criterion(u_exact_, u_pred_)
