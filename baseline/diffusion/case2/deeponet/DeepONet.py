...
def F(x, case_index):
    u_exact_file = os.path.join('../../../../data/diffusion/case2/train_data', f'res{case_index + 1}.vtu')
...
spatial_coords = torch.from_numpy(spatial_coords).float().to(device)
f_data = torch.from_numpy(f_data).float().to(device)
BC_data = torch.from_numpy(BC_data).float().to(device)
targets = torch.from_numpy(targets).float().to(device)

targets_mean = torch.from_numpy(targets_mean).float().to(device)
targets_std = torch.from_numpy(targets_std).float().to(device)
...
# 创建DeepONet
deeponet = DeepONet(branch_layers1, branch_layers2, trunk_layers).cuda()
